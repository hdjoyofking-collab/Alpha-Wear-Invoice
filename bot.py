...
# Conversation States
INV_NUM, INV_DATE, NAME, PHONE, ADDRESS, P_NAME, SIZE, PRICE, QTY, DISCOUNT = range(10)

def draw_decorations(canvas, doc):
    canvas.saveState()
    # Fixed Top Banner Graphics
    ...
    # Fixed Bottom Banner Graphics (At the absolute bottom of A4)
...
def create_invoice(inv_num, inv_date, cust_name, cust_phone, cust_address, item_name, item_size, price, qty, discount):
    buffer = io.BytesIO()
    ...
    if os.path.exists("logo.png"):
        logo_img = Image("logo.png", width=50, height=50)
        logo_img.hAlign = 'LEFT'
        brand_header.append(logo_img)
        brand_header.append(Spacer(1, 4))
    
    brand_header.append(Paragraph("<b>ALPHA WEAR</b>", brand_title_style))
    ...
    desc_str = f"{item_name} ({item_size})" if item_size.strip() and item_size.upper() != 'N/A' else item_name
    tot = qty * price
    subtotal = tot
...
async def start_form(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.message.reply_text("🔢 **১/১০: ইনভয়েস নম্বর (Invoice No.) লিখুন:**")
    return INV_NUM

async def get_inv_num(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['inv_num'] = update.message.text.strip()
    await update.message.reply_text("📅 **২/১০: তারিখ (Date) লিখুন (যেমন: 2026-08-28):**")
    return INV_DATE
...
async def get_inv_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("👤 **৩/১০: কাস্টমারের নাম লিখুন:**")
    return NAME

async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['name'] = update.message.text.strip()
    await update.message.reply_text("📞 **৪/১০: কাস্টমারের মোবাইল নম্বর দিন:**")
    return PHONE

async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['phone'] = update.message.text.strip()
    await update.message.reply_text("📍 **৫/১০: কাস্টমারের পূর্ণাঙ্গ ঠিকানা দিন:**")
    return ADDRESS

async def get_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['address'] = update.message.text.strip()
    await update.message.reply_text("👕 **৬/১০: প্রোডাক্টের নাম লিখুন:**")
    return P_NAME

async def get_p_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['p_name'] = update.message.text.strip()
    await update.message.reply_text("📏 **৭/১০: প্রোডাক্টের সাইজ লিখুন (যেমন: M, L, XL, XXL) (না থাকলে 'N/A' লিখুন):**")
    return SIZE

async def get_size(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data['size'] = update.message.text.strip()
    await update.message.reply_text("💰 **৮/১০: প্রতি পিস প্রোডাক্টের প্রাইস (টাকা):**")
    return PRICE

async def get_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        context.user_data['price'] = float(update.message.text.strip())
        await update.message.reply_text("📦 **৯/১০: মোট কত পিস নিচ্ছেন (পরিমাণ):**")
        return QTY
    except ValueError:
...
async def get_qty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("🏷️ **১০/১০: ডিসকাউন্ট প্রাইস (টাকা) (ডিসকাউন্ট না থাকলে '0' লিখুন):**")
        return DISCOUNT
    except ValueError:
...
async def get_discount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        pdf_file = create_invoice(inv_num, inv_date, name, phone, address, p_name, size, price, qty, discount)
